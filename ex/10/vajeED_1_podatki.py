...
def generiraj_newton(primeri, noise=0):
...
def generiraj_stefan(primeri, noise=0):
...
def generiraj_lorenz(primeri, noise=0):
    ...
    for i in range(primeri):
        ...
        gama = np.sqrt(1 - (v / c) ** 2) * (1 + np.random.normal(0, noise))
        podatki["gama"].append(gama)
        podatki["v"].append(v)
    return pd.DataFrame(podatki)


def generiraj_energijski_zakon_const(primeri, noise=0):
...
def generiraj_energijski_zakon(primeri, noise=0):
    """Generira podatke za m g h + 0.5 m v^2 = c.
    ...
    for i in range(primeri):
        ...
        podatki["v"].append(v)
        podatki["h"].append(h)
        podatki["E"].append((m*g*h + 0.5*m*v**2)*(1 + np.random.normal(0, noise)))
    return pd.DataFrame(podatki)


def generiraj_ploscina(primeri, noise=0):
    """Generira podatke za p = n a ** 2 / (4 tan (pi / n))"""
    np.random.seed(1234)
    podatki = {"n": [], "a": [], "p": []}
    for n in range(3, primeri + 3):
        a = np.random.rand(1)[0]
        p = n * a ** 2 / (4 * np.tan(np.pi / n)) * (1 + np.random.normal(0, noise))
        podatki["n"].append(n)
        podatki["a"].append(a)
        podatki["p"].append(p)
    return pd.DataFrame(podatki)
